...
class TestOperators(object):
    ...
    def test_dot_product(self):
        ...
        zero = mx.nd.array([0.])
        one = mx.nd.array([1.])
        model.Z = Normal.define_variable(mean=broadcast_to(zero, (m, k)), variance=broadcast_to(one, (m, k)), shape=(m, k))
        model.D = Normal.define_variable(mean=broadcast_to(zero, (k, n)), variance=broadcast_to(one, (k, n)), shape=(k, n))
        sigma_2 = Variable(shape=(1,), transformation=PositiveTransformation())
        model.X = Normal.define_variable(mean=dot(model.Z, model.D), variance=broadcast_to(sigma_2, (m, n)), shape=(m, n))
